...
import { ResourceModel } from '../components/Resources/ResourcesModel';
import { setFetchResults } from '../objects/Mock';
import { preprocessor } from '../objects/DatabaseInterface';

describe("ResourceModel test", () => {
    const model = new ResourceModel();
    test('ResourceModel initRecord test', () => {
        setFetchResults([]);
        let res = model.initRecord();
        expect(res.id).toBe('');
    });
    test('ResourceModel getRecord test OK', () => {
        setFetchResults([
    ...
    test('ResourceModel getRecord test notfound', () => {
        setFetchResults([
    ...
    test('ResourceModel getRecords test', () => {
        setFetchResults([
            { data:{records:[{id:'123','name':'name123'}, 
                            {id:'124','name':'name124'}]}}
        ]);
    ...
    test('ResourceModel getTotal test', () => {
        setFetchResults([
    ...
    test('ResourceModel save / insert test', () => {
        setFetchResults([
            {data:[{ id: '1'}]} 
        ]);
    ...
    test('ResourceModel save / update test', () => {
        setFetchResults([
            {data:[{ id: '123'}]} 
        ]);
        let record = model.initRecord();
    ...
    test('ResourceModel delete test', () => {
        setFetchResults([
            {data:[{ affected_rows: 10}]}, // delete labels
